...
class RETAIN(nn.Module):
    def __init__(self, dict_len_list, dim_input=256, dim_emb=256, dropout_input=0.8, dropout_emb=0.9, dim_alpha=256,
...
def main():
    ...
    train_auroc = roc_auc_score(torch.cat(labels, 0).numpy(), torch.cat(outputs, 0).numpy()[:, 1])
    train_auprc = average_precision_score(torch.cat(labels, 0).numpy(), torch.cat(outputs, 0).numpy()[:, 1])
    ...
    for b_i, batch in enumerate(tqdm(test_loader)):
        ...
        test_outputs.append(F.softmax(output).data)
        # print("pred_labels.size: ", pred_labels.size())
        # print("pred_labels: ", pred_labels.float().tolist())
        total_loss += loss.item()
    ...
    print("Epoch: ", e_i+1, "Average Loss: ", total_loss / test_count)

    test_auroc = roc_auc_score(torch.cat(test_labels, 0).numpy(), torch.cat(test_outputs, 0).numpy()[:, 1])
    test_auprc = average_precision_score(torch.cat(test_labels, 0).numpy(), torch.cat(test_outputs, 0).numpy()[:, 1])
